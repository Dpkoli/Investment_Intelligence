...
def _render_detail_panel(p, prices: dict) -> None:
    ...
    st.markdown(
        f"""
<div style="background:#1A1D24;border:1px solid #2E3140;border-left:4px solid {color};
        ...
        with col:
            st.markdown(
                f"""<div style="background:#1A1D24;border:1px solid #2E3140;border-left:3px solid {color};
                    ...
                    <div style="font-size:0.68rem;color:#888;margin-top:0.2rem">troy oz · {ticker}</div>
                </div>""",
                unsafe_allow_html=True,
            )
